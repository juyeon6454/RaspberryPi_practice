...
void saveImage(unsigned char *inimg)
{
   ...
   bmpHeader.bf0ffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER); // 54;
   bmpHeader.bf0ffBits += sizeof(RGBQUAD) * 256; 
...
static void processImage(const void *p)
{
    ...
    for(y = 0; y < height; y++) {
        for(j = 0, x = 0; j < vinfo.xres_virtual * 2; j += 4, x += 2) {
